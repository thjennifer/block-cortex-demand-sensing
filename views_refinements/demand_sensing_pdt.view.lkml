include: "/views/demand_sensing_pdt.view"
    ...
    sql:(WITH
        ...
        SELECT
          ...
          FROM
            date) AS week
        FROM
             UNNEST(GENERATE_DATE_ARRAY(DATE_ADD((@{use_for_current_date}), INTERVAL -cast( @{years_of_past_data}  as INT64) YEAR), DATE_ADD(@{use_for_current_date}, INTERVAL 13 WEEK))) AS date ),
